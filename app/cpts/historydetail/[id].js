...
export default function historyClass() {
    ...
    const renderItem = ({ item }) => {
        const handlePress = () => {
            router.push({
                pathname: `cpts/feedback/${item.id_attendancedetail}`
            });
        };
                    ...
                    <View>
                        ...
                        {data &&
                            <View>
                                ...
                                <View style={{ backgroundColor: COLORS.light, paddingHorizontal: 20, paddingVertical: 10, marginTop: 10, borderRadius: 10, flexDirection: 'row', justifyContent: 'space-between' }}>
                                    <Text style={{color: COLORS.grey}}>{data.id_instructor?.name}</Text>
                                    <Text style={{ color: COLORS.darkBlue, fontWeight: 'bold' }} onPress={() => router.push({
                                        pathname: `cpts/review/${data.id_attendance}`
                                    })}>See Feedback</Text>
                                </View>
                                </View>
