...
export default function classhistory() {
  ...
  const fetchData = async () => {
    try {
      const response = await authenticatedRequest(`/api/v1/instructor/attendancebyidtrainingclass/${id}`, "GET");
  ...
  const renderItem = ({ item }) => {
    ...
    const handlePress = () => {
      router.push({
        pathname: `instructor/historydetail/${item.id_attendance}`,
        params: { name: item.id_trainingclass.name}
      });
    };

    return (
      <CardTraining
        text={` ${formatDate(item.date)} - ${formatDate(item.valid_to)}`}
        subtext={item.status}
